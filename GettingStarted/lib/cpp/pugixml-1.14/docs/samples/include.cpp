...
// tag::code[]
bool load_preprocess(xml_document& doc, const char* path);

bool preprocess(xml_node node)
{
    for (xml_node child = node.first_child(); child; )
    {
        if (child.type() == node_pi && strcmp(child.name(), "include") == 0)
        {
            xml_node include = child;
            ...
            node.insert_child_before(node_comment, include).set_value(path);

            // copy the document above the include directive (this retains the original order!)
            for (xml_node ic = doc.first_child(); ic; ic = ic.next_sibling())
            {
...
bool load_preprocess(xml_document& doc, const char* path)
{
    xml_parse_result result = doc.load_file(path, parse_default | parse_pi); // for <?include?>
