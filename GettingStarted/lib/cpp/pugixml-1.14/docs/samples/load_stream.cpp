...
void print_doc(const char* message, const xml_document& doc, const xml_parse_result& result)
{
    std::cout
        << message
        << "\t: load result '" << result.description() << "'"
        << ", first character of root name: U+" << std::hex << std::uppercase << std::setw(4) << std::setfill('0') << as_wide(doc.first_child().name())[0]
